    ...
    pub fn parse() -> anyhow::Result<Self> {
        ...
        let dma_channels = files
            .map(|f| {
                ...
                let parsed: xml::Ip =
                    quick_xml::de::from_str(&std::fs::read_to_string(&f)?).context(format!("{:?}", f))?;
                ...
                for dma in parsed.mode_logic_operator.modes {
                    ...
                    if channels.len() == 1 {
                        ...
                        for n in dma_peri_name.split(',') {
                            let n = n.trim();
                            let re =
                                regex::Regex::new(&format!(".*{n}{}", r"_(Channel|Stream)\[(\d+)-(\d+)\]")).unwrap();
        ...
        let gpdma_channels = files.map(|(file, instance, version, count, count_2d)| {
            let mut chip_dma = ChipDma {
                peripherals: HashMap::new(),
                channels: Vec::new(),
            };
            ...
            Ok((format!("{version}:{instance}"), chip_dma))
        });
